...
| First DB-backed test run after a non-clean incremental build fails with `ServiceConfigurationError: io.ebean.config.EntityClassRegister ... not found` | Maven incremental compile drops the querybean-generator service registration when no entity source changed | run `mvn clean test` once and continue; after adding/removing classes, prefer clean over incremental |
