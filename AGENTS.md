...
1. **Check-main-first hook**: Run `powershell scripts/check-main-first.ps1`. Exit code 0 = pass, 2 = fail (unmerged changes or not on main).
2. **Create-pr hook**: Run `powershell scripts/create-pr.ps1`. Exit code 0 = pass (PR created or not needed), 2 = fail (error).
3. **Integration test**: Make a change, commit it, then run the hooks to verify they trigger correctly.
